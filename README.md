...
Ils sont utilisés pour gérer toute sorte de scénarios liés à la navigation, rediriger un utilisateur qui tente d'accéder à une route, l'obliger à s'authentifier pour accéder à certaines fonctionnalités. Les guards reposent sur un mécanisme assez simple, ils retournent un booléen qui permet de contrôler le comportement de la navigation. Si le guard retourne "true", le processus de navigation continue, si il retourne "false", le processus de navigation cesse et l'utilisateur reste sur la même page. Un guard peut retourner un booléen de manière synchrone ou asynchrone mais dans la plupart des cas, un guard ne peut pas renvoyer un résultat de manière synchrone car il doit attendre une réponse. Il peut être nécessaire de poser une question à l'utilisateur, de sauvegarder des changements, de récupérer des données plus récentes depuis le serveur, toutes ces actions sont asynchrones. Dans la plupart des cas, le type de retour d'un guard est un Observable qui renvoit un booléen ou une promesse et le routeur attendra la réponse pour agir sur la navigation. Même si un gard est conçu pour interagir avec la navigation, il en existe des types différents. Le guard "CanACtivate" peut influencer sur la navigation d'une route et notamment la bloquer, c'est ce type de guard qu'on va utiliser pour construire un système d'authentification dans l'application. Le guard "CanActivateChild" peut influencer sur la navigation d'une route fille. Le guard "CanDeactivate" peut empêcher l'utilisateur de naviguer en dehors de la route courante, cela peut s'avérer utile pour valider un formulaire avant de continuer la navigation. Le guard "Resolve" peut effectuer une récupération de données avant de naviguer. Le guard "CanLoad" peut gérer la navigation vers un sous-modules chargé de manière asynchrone. On peut avoir différents guards à tous les niveaux du système de navigation mais si un guards, à un moment, retourne "false", tous les autres "guards" en attente seront annulés et la navigation entière sera bloquée.

## Mettre en place un guard 
Les applications ont souvent besoin de restreindre l'accès aux fonctionnalités en fonction de l'utilisateur : obliger l'utilisateur à s'authentifier pour accéder à son espace membre, il faut alors bloquer ou limiter l'accès jusqu'à ce que l'utilisateur. On va se contenter d'afficher un message dans le navigateur lorsque l'utilisateur essaye d'accéder à la page d'édition d'un pokémon car il s'agit d'une route sensible, on ne veut pas que tout le monde puisse modifier les pokémons. On va utiliser un guard de type "activate", on va développer ce guard dans le fichier "auth-gard.service.ts" dans le dossier "app". Si on clique sur "Editer" de la fiche pokémon, une fois l'application démarée, le guard intercepte la tentative d'accéder à la page d'édition du pokémon et si on clique sur "ok", le guard autorise d'accéder à cette page car il retourne "true" par défaut.
...
On peut déployer l'application en copiant tous les fichiers du projet vers le serveur distant mais ce n'est pas une méthode très optimisée : l'application ne sera pas rapide pour l'utilisateur et Firebase Hosting dispose d'une capacité limitée dans la version gratuite. On doit effectuer un certain nombre d'opérations sur le projet avant de le déployer, il faudrait voir comment souscrire un compte chez un hébergeur, toutes les techniques d'optimisation possibles avant le déploiement, comment configurer le serveur de production, etc. Ce qu'on va voir, c'est quels sont les éléments intéressants à optimiser avant le déploiement sur Firebase Hosting, l'objectif est fonctionnel, le site sera en ligne mais on verra pas comment souscrire à un hébergeur classique et déployer notre application. On va voir les bases pour déployer un site statique ne contenant que des fichiers HTML, CSS et JavaScript et on pourra déployer notre archive sur un hébergeur. 

## Tâches 
1. Préparer le projet en local avant de le déploiement 
2. Créer le projet sur le site de Firebase, cela donnera accès à une console d'administration dont on pourra consulter l'historique des déploiements, revenir à une version plus ancienne du projet 
3. On déploiera l'application sur Firebase
...
Il faut d'abord créer un projet dans Firebase, cela donnera accès à une console d'administration pour le projet. Ensuite, on devra installer Firebase cli en local, c'est une boîte à outil mis à disposition par Firebase pour permettre de déployer l'application en une seule ligne de commande sur le serveur, on devra ensuite configurer le projet auprès de Firebase pour préparer les éléments spécifiques au déploiement et on déploiera l'application de Pokémon et on pourra y accéder sur Internet. On va se connecter à l'interface d'administration de Firebase, il faut entrer nos identifiants "google" si on est pas déjà connecté : https://firebase.google.com/ et se connecter avec un compte google. Aller à "Accéder à la console" : https://console.firebase.google.com/?pli=1, il y a un bouton qui permet d'ajouter un projet, on clique et on doit choisir un nom pour l'application, crée un compte et chosir le pays, ensuite on arrive dans un espace dédié à l'administration de l'application. Dans le menu à gauche de l'interface, il y a un onglet "Hosting", qui est un service qu'on va utiliser et c'est à cet endroit qu'on pourra consulter l'historique des déploiements sur Firebase, on a maintenant un projet disponible sur Firebase prêt à accueillir les fichiers de l'application. 

## CLI : Command Line Inteface (interface en ligne de commande)
Il faut utiliser l'outil fournit par Firebase, Firebase CLI, il permet de déployer facilement des applications sur Firebase. Cela permet d'exécuter certaines commandes de Firebase depuis un terminal, pour l'installer, il faut taper la commande "npm install -g firebase-tools". Le "-g" permet d'installer l'utilitaire de Firebase, au niveau global sur la machine, on pourra le réutiliser pour d'autres projets, cela peut prendre quelques minutes à s'exécuter, le temps de télécharger tous les éléments nécessaires. Pour mettre à jour CLI, il faut taper à nouveau la même commande. Une fois l'installation terminée, on peut exécuter les commandes de Firebase CLI, avant de l'utiliser pour déployer l'application, il faut exécuter 2 commandes : "firebase --version" pour vérifier que l'utilitaire est bien installé sur la machine, cela indique la version d'utilitaire de Firebase et "firebase login", instruction qui permet de lier le compte Google et les projets Firebase à l'utilitaire qu'on a installé. Il faut suivre les instructions dans le terminal jusqu'au message qui dit que le compte Google est lié à l'utilitaire de Firebase, on est donc bien connecté. 

## Déployer
Relancer les commandes "npm install" et "npm start" pour être sur que tout fonctionne correctement en local. On va à nouveau utliser la commande Firebase CLI pour configurer le projet avant le déploiement. On va dire à Firebase que le dossier local de la machine doit correspondre au projet qu'on a crée sur Firebase et on veut déployer les fichiers sur le serveur distant de Firebase, à l'aide de la commade "firebase init". Il faut se placer à la racine du projet et on exécute cette commande, un certain nombre d'éléments vont être demandé. 

- Firebase demande quel service de Firebase CLI on souhaite utiliser : on souhaite uniquement utiliser la fonction "hosting", on ne se servira que de cette fonctionnalité proposée par Firebase pour le déploiement (sélectionner hosting, appuyer sur espace pour choisir "hosting" et puis Enter).
- Firebase demande quel projet on souhaite associer, déplacer le curseur sur le nom du projet crée.
- Firebase veut savoir le chemin relatif du dossier du projet à déployer, c'est le projet contenant le fichier "index.html", on va donc mettre "src" pour signaler à Firebase pour signaler que le fichier "html" est dans ce dossier.
- Firebase propose de s'occuper de configurer les redirections sur le serveur : taper "y" pour yes, on verra ou Firebase écrit ces règles.
- Firebase demande si il peut compresser le fichier "index.html" : taper "n" pour non et Enter pour refuser l'écrasement du fichier "index.html".

L'utilitaire crée 2 fichiers : ".firebaserc" et "firebase.json", c'est le deuxième qui nous intéresse, il a été créé en fonction des réponses qu'on a donné, on peut vérifier que les règles de redirection vers le fichier "index.html" sont bien présentes, c'est une configuration du serveur indispensable pour que l'application fonctionne correctement. Si on souhaite déployer sur d'autres serveurs, il y a une ressource en description pour adapter la configuration du serveur : https://angular.io/guide/deployment#server-configuration. Sur la documentation officiel de Firebase, pour la configuration du fichier "firebase.json", on indique qu'il est possible d'ajouter une option "ignore" qui indique à Firebase CLI, les fichiers ou dossiers qu'on ne veut pas déployer sur les serveurs de production, on fait le configurer en fonction pour ne pas déployer les fichiers dans sources, le fichier "firebase.json" qui concerne uniquement la configuration et tous les fichiers cachés qui commencent par un "." : le dossier "node modules" et le dossier "src" qui contient le code de l'applicaiton, on veut juste déployer le dossier "dist". La plupart des fichiers ont déjà étés ignorés par le fichier de configuration de Firebase, il faut juste ajouter le fait de devoir ignorer le dossier source. Il faut sauvegarder les modifications et on prêt pour le déploierment. Le fichier ."firebasesrc" permet de configurer certains éléments, pour l'instant, on a juste le nom de l'application.
